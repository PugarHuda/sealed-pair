...
| 2:10–2:25 | Open `/api/mcp` in browser tab → JSON tool catalog. Then `.mcp.json` in repo. | "Read-only MCP tools expose the board, maker stats, and the digest verifier to any AI agent — `list_open_orders`, `verify_settle_digest`, `maker_stats`. The `.mcp.json` config drops straight into Claude Desktop." |
